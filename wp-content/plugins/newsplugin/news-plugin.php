...
//[feed_shortcode title="" keywords="News" count="" age="" sources="" excluded_sources="" search_mode="" search_type="" sort_mode="" link_type="" show_date="" show_source="" show_abstract="" feed_mode=""]
    ...
    function register_styles() {
        wp_register_style( 'news-plugin', plugins_url( 'newsplugin/assets/css/news-plugin.css' ), array(), "0.1" );
        wp_enqueue_style( 'news-plugin' );
    }

    function register_admin_scripts() {
        $assets_path = plugins_url( 'newsplugin/assets/');
   ...
   function admin_menu() {
	...
	function get_with_default($arr, $a, $b, $def) { /* Grrr this should be language construct ... oh. It will be. PHP 7. https://wiki.php.net/rfc/isset_ternary */
		if(!is_array($arr)) {return $def;}
		if(!isset($arr[$a])) {return $def;}
		if(!isset($arr[$a][$b])) {return $def;}
		return $arr[$a][$b];
	}
	...
	function newsplugin_options_page() {
		...
		<div class="wrap">
            ...
            <script type="text/javascript">
				...
				function generateShortcode() {
					...
					var key;
					var str_opts = new Object({newsplugin_title: 'title', newsplugin_keywords: 'keywords', newsplugin_search: 'search_mode', newsplugin_sort: 'sort_mode', newsplugin_link_open: 'link_open_mode', newsplugin_link_follow: 'link_follow', newsplugin_publishing: 'feed_mode'});
					for (key in str_opts) {
					...
					var bool_opts = new Object({newsplugin_more_dates: 'show_date', newsplugin_more_sources: 'show_source', newsplugin_more_abstracts: 'show_abstract'});
		...
		<input type="hidden" name="action" value="news_plugin_send_feedback">
			...
			<h3>From</h3><?php
				...
				$v = empty($content) ? '' : ' checked="checked"';
